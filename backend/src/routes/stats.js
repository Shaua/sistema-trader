...
router.get('/dashboard', authMiddleware, async (req, res) => {
  try {
    const kpis = await statsEngine.calculateDashboardKPIs(req.userId);
    if (!kpis) {
...
router.get('/charts', authMiddleware, async (req, res) => {
  try {
    ...
    const charts = await statsEngine.getChartData(req.userId, period);
    res.json(charts);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/alerts', authMiddleware, async (req, res) => {
  try {
    const alerts = await statsEngine.checkRiskAlerts(req.userId);
    res.json(alerts);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/insights', authMiddleware, async (req, res) => {
  try {
    const insights = await statsEngine.generateInsights(req.userId);
    res.json(insights);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
